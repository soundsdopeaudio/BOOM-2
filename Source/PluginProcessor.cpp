...
void BoomAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    // 1) If Rhythmimick/Beatbox recording is active, append the incoming audio
    //    into our mono capture ring buffer BEFORE we clear anything.
    if (isCapturing.load() && buffer.getNumSamples() > 0)
        appendCaptureFrom(buffer);

    // 2) BOOM doesn’t output audio (we generate/export MIDI),
    //    so keep the plugin silent.
    buffer.clear();

    // 3) We’re not emitting live MIDI here. If you later want to stream
    //    pattern MIDI during playback, this is where you’d fill 'midi'.
