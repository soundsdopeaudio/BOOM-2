...
{
    // Only act if Drums engine is selected, otherwise do nothing.
    auto eng = getEngineSafe();
    if (eng != boom::Engine::Drums)
        return;

    // How many visible drum rows do we have?
    const auto rows = getDrumRows();           // StringArray you already expose
    const int  N = rows.size();
    if (N <= 1) return;

    // Move each note's row index up by 1; top wraps to bottom.
    auto pat = getDrumPattern();               // your Pattern (Array<Node> or similar)
    for (auto& n : pat)
        n.row = (n.row + 1) % N;
...
void BoomAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    if (auto* ph = getPlayHead())
    {
        ...
        if (ph->getCurrentPosition(pi))
            if (pi.bpm > 0.0)
                lastHostBpm.store(pi.bpm);
    }
