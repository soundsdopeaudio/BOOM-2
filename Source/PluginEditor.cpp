    ...
    drumGrid(p), pianoRoll(p)
{
    ...
    rest808.setRange(0, 100); rest808.setTextBoxStyle(juce::Slider::NoTextBox, false, 10, 0);
    ...
    btnFlippit.onClick = [this]
    {
        ...
        flippit.reset(new FlippitWindow(
            ...
            [this](int density)
            {
                auto eng = (boom::Engine)proc.apvts.getRawParameterValue("engine")->load();
    ...
    {
        // Create the content and force a smaller initial size *before* handing it to LaunchOptions.
        auto* content = new AIToolsWindow(proc, [this] { aiTools.reset(); });
        content->setSize(896, 818);   // <-- smaller than before; change if you want

        aiTools.reset(content);       // keep a strong pointer so we can close it from inside

        juce::DialogWindow::LaunchOptions o;
        o.content.setOwned(aiTools.release()); // LaunchOptions takes ownership
        o.dialogTitle = "A.I. TOOLS";
        o.useNativeTitleBar = true;
        o.resizable = true;                  // resizable window; OS frame provides resizing
        // (No dialogWidth/Height, no useBottomRightCornerResizer, no constrainer)
...
void BoomAudioProcessorEditor::resized()
{
    ...
    useTriplets.setBounds(S(690, 10, 20, 20));
    tripletDensity.setBounds(S(628, 32, 30, 20));
    dottedNotesLblImg.setBounds(S(565, 45, 114, 26));
    dottedDensity.setBounds(S(626, 65, 30, 20));
    useDotted.setBounds(S(690, 40, 20, 20));
    ...
    y += spacing;
    humanizeTiming.setBounds(S(right_x, y, 125, 26));
    y += spacing;
    humanizeVelocity.setBounds(S(right_x, y, 125, 26));
    y += spacing;
    swing.setBounds(S(right_x, y, 125, 26));
...
void AIToolsWindow::resized()
{
    ...
    // Title + "Select a tool"
    titleLbl.setBounds(S(160, 24, 760, 120));
    selectAToolLbl.setBounds(S(1300, 80, 200, 40));

    // ---- Rhythmimick row (top block)
    rhythmimickLbl.setBounds(S(420, 190, 360, 60));
    rhythmimickDescLbl.setBounds(S(420, 245, 500, 46));
    recordUpTo60LblTop.setBounds(S(90, 265, 280, 30));

    btnRec1.setBounds(S(90, 230, 52, 52));
    btnStop1.setBounds(S(160, 230, 52, 52));
    ...
    // ---- Slapsmith row
    slapsmithLbl.setBounds(S(420, 390, 360, 60));
    slapsmithDescLbl.setBounds(S(420, 445, 520, 46));
    ...
    // ---- Style Blender row
    styleBlenderLbl.setBounds(S(420, 585, 360, 60));
    styleBlenderDescLbl.setBounds(S(420, 640, 560, 46));
    styleA.setBounds(S(84, 645, 280, 20));
    styleB.setBounds(S(84, 705, 280, 20));
    ...
    // ---- Beatbox row (bottom)
    beatboxLbl.setBounds(S(420, 820, 360, 60));
    beatboxDescLbl.setBounds(S(420, 875, 560, 110));
    recordUpTo60LblBottom.setBounds(S(90, 905, 280, 30));
    btnRec4.setBounds(S(90, 870, 52, 52));
    btnStop4.setBounds(S(160, 870, 52, 52));
    btnGen4.setBounds(S(80, 955, 150, 46));
    btnSave4.setBounds(S(240, 955, 150, 46));
    btnDrag4.setBounds(S(400, 955, 170, 46));
    ...
    : proc(p), onCloseFn(std::move(onClose)), onFlipFn(std::move(onFlip))
{
    ...
    variation.setTextBoxStyle(juce::Slider::NoTextBox, false, 0, 0);
    ...
    : proc(p), onCloseFn(std::move(onClose)), onGenerateFn(std::move(onGen))
{
    ...
    rollsTitleImg.setImage(loadSkin("rollGeneratorLbl.png")); addAndMakeVisible(rollsTitleImg);
    addAndMakeVisible(styleBox); styleBox.addItemList(boom::styleChoices(), 1); styleBox.setSelectedId(1);
    addAndMakeVisible(barsBox);  barsBox.addItemList(juce::StringArray("1", "2", "4", "8"), 1); barsBox.setSelectedId(2);
    ...
    btnGenerate.onClick = [this] { if (onGenerateFn) onGenerateFn(styleBox.getText(), barsBox.getSelectedId(), (int)variation.getValue()); };
