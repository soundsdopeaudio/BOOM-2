...
void BoomAudioProcessorEditor::resized()
{
    ...
    {
        auto drumArea = S(60, 360, 980, 340); // ← YOUR original, keep it unchanged
        drumGridView.setBounds(drumArea);
        drumGrid.setTopLeftPosition(0, 0);
        drumGrid.setSize(drumArea.getWidth(), drumArea.getHeight());
    }

    // PIANO ROLL (main window) — exact size
    {
        auto rollArea = S(60, 740, 980, 320); // ← YOUR original, keep it unchanged
        pianoRollView.setBounds(rollArea);
        pianoRoll.setTopLeftPosition(0, 0);
        pianoRoll.setSize(rollArea.getWidth(), rollArea.getHeight());
    }
...
{
    activeTool_ = t;
    auto set = [](juce::ImageButton& b, bool on) { b.setToggleState(on, juce::dontSendNotification); };

    set(toggleRhythm, t == Tool::Rhythmimick);
    set(toggleSlap, t == Tool::Slapsmith);
    set(toggleBlend, t == Tool::StyleBlender);
    set(toggleBeat, t == Tool::Beatbox);
    ...
    btnRec1.setEnabled(r);   btnStop1.setEnabled(r);   btnGen1.setEnabled(r);   btnSave1.setEnabled(r);   btnDrag1.setEnabled(r);
    btnGen2.setEnabled(s);   btnSave2.setEnabled(s);   btnDrag2.setEnabled(s);  miniGrid.setEnabled(s);
    btnGen4.setEnabled(b);   btnRec4.setEnabled(b);    btnStop4.setEnabled(b);  btnSave4.setEnabled(b);   btnDrag4.setEnabled(b);
...
void AIToolsWindow::resized()
{
    ...
    selectAToolLbl.setBounds(S(550, 50, 120, 40));

    // ---- Rhythmimick row (top block) ----
    int y_offset = 130;
    rhythmimickLbl.setBounds(S(300, y_offset, 220, 30));
    ...
    rhythmSeek.setBounds(S(90, y_offset + 20, 140, 30));
    ...
    // ---- Slapsmith row ----
    y_offset = 270;
    slapsmithLbl.setBounds(S(300, y_offset, 220, 30));
    ...
    // ---- Style Blender row ----
    y_offset = 410;
    styleBlenderLbl.setBounds(S(300, y_offset, 220, 30));
    toggleBlend.setBounds(S(550, y_offset, 120, 40));
    styleBlenderDescLbl.setBounds(S(300, y_offset + 35, 250, 40));

    // Style A/B combo boxes (same as before)
    styleABox.setBounds(S(84, 610, 280, 28));
    styleBBox.setBounds(S(84, 670, 280, 28));

    // One blended slider between them
    blendSlider.setBounds(S(84, 705, 280, 20)); // below the boxes; adjust to taste

    // Arrows between toggles and the text blocks (adjust Y to your art)
    arrowRhythm.setBounds(S(1200, 220, 80, 40));
    arrowSlap.setBounds(S(1200, 420, 80, 40));
    arrowBlend.setBounds(S(1200, 620, 80, 40));
    arrowBeat.setBounds(S(1200, 920, 80, 40));

    // Slapsmith mini-grid location (already there, just leave or tweak)
    miniGrid.setBounds(S(600, 480, 880, 200));  // your existing small rect
    auto mg = miniGrid.getBounds();
    miniGrid.setTopLeftPosition(0, 0);          // inside its own viewport, origin
    miniGrid.setSize(mg.getWidth(), mg.getHeight()); // <- FORCE SIZE
    ...
    // ---- Beatbox row (bottom) ----
    y_offset = 550;
    beatboxLbl.setBounds(S(300, y_offset, 220, 30));
    toggleBeat.setBounds(S(550, y_offset, 120, 40));
    beatboxDescLbl.setBounds(S(300, y_offset + 35, 250, 40));

    recordUpTo60LblBottom.setBounds(S(50, y_offset + 50, 180, 20));
    btnRec4.setBounds(S(50, y_offset + 20, 30, 30));
    beatboxSeek.setBounds(S(90, y_offset + 20, 140, 30));
    btnStop4.setVisible(false);
