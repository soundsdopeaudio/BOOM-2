...
public:
    ...
    void flipMelodic(int densityPct, int addPct, int removePct);  // for Flippit (808/Bass)
    void flipDrums(int densityPct, int addPct, int removePct);     // for Flippit (Drums)
    ...
    double getHostBpm() const noexcept { return lastHostBpm.load(); }
...
// Returns a vector<int> length = bars*16, values in [0..100] = probability boost.
    ...
    // Simple transforms
// ---- Bumppit (Drums path + Melodic path) ----
    void bumpDrumRowsUp();                                                     // DRUMS ONLY
...
private:
    ...
    // ---- Quantize helpers ----
    static constexpr int PPQ = 96;     // 96 ticks/quarter (we used this elsewhere)
